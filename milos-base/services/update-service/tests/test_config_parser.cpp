...
package_management:
  type: pacman
    ...
    createConfigFile(configContent);
    QCOMPARE(QString::fromStdString(m_configParser->getString("package_management.type")), QString("pacman"));
}
...
signature_verification:
  enabled: false
    ...
    QVERIFY(!m_configParser->getBool("signature_verification.enabled", true));
}
...
update_policy:
  check_interval_hours: 24
    ...
    createConfigFile(configContent);
    QCOMPARE(m_configParser->getInt("rollback.max_snapshots", 0), 10);
    QCOMPARE(m_configParser->getInt("update_policy.check_interval_hours", 0), 24);
}
